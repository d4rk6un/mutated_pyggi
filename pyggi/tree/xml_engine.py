...
class XmlEngine(AbstractTreeEngine):
    ...
    #ADD
    @classmethod
    def get_function_variables(cls, contents_of_file):
        function_variables = dict()
        function_count = 1
        for func in contents_of_file.findall('.//function'):
            func_name = f'function[{function_count}]'
            func_vars = []
            for p_list in func:
                if p_list.tag == 'parameter_list':
                    for var in p_list.findall('.//decl/name'):
                        func_vars.append(var.text)

            function_variables[func_name] = func_vars
            function_count += 1
    ...
    #ADD
    @classmethod
    def random_target_variable(cls, program, op, new_contents, modification_points):
        variables = program.variables[op.target[0]]
        m_points = modification_points[op.target[0]]
        contents = new_contents[op.target[0]]
        candidate = []

        for index, point in enumerate(m_points):
            for func_name, vars in variables.items():
                if func_name + '/block' in point:
                    if contents.find(point) is not None and contents.find(point).text in vars:
                        v = copy.deepcopy(vars)
                        if len(v) > 1:
                            v.remove(contents.find(point).text)
                        candidate.append((index, v))

        random_target = random.choice(candidate)
        random_variable = random.choice(random_target[1])

        return (op.target[0], random_target[0]), random_variable

    #ADD
    @classmethod
    def random_target_cmp_oper(cls, program, op, new_contents, modification_points):
        cmp_opers = program.cmp_opers
        m_points = modification_points[op.target[0]]
        contents = new_contents[op.target[0]]
        candidate = []

        for index, point in enumerate(m_points):
            match = re.search(r'operator\[[1-9]\]$', point)
            if match:
                if contents.find(point) is not None and contents.find(point).text in cmp_opers:
                    c = copy.deepcopy(cmp_opers)
                    c.remove(contents.find(point).text)
                    candidate.append((index, c))

        random_target = random.choice(candidate)
        random_cmp_oper = random.choice(random_target[1])

        return (op.target[0], random_target[0]), random_cmp_oper

    #ADD
    @classmethod
    def do_replace_text(cls, target, text, new_contents, modification_points):
        # get elements
        target_element = new_contents[target[0]].find(modification_points[target[0]][target[1]])

        if target_element is None or text is None:
            return False
